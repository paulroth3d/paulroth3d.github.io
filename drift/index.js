window.onReady = function onReady() {
  ...
  const scaleCoef = (Math.round(data.scale) || 100) / 100;
...
SVG.on(document, 'DOMContentLoaded', function() {
  ...
  const data = {
    ...
    //-- 100% scale for mapping the coordinates
  ...
  data.scale = Number.parseInt(data.scale) || 100;
...
* {Integer} scale - 100% default - zoom on the noise region
...
`);
  ...
  initializeInput(form, 'scale', data.scale);
